...
func TestEncrypt(t *testing.T) {
	...
	sk, err = aes_cbc_256.NewCrypto(sk_ei, iv_nil, padding_nil)
	require.NoError(t, err)
	cipher, err = sk.Encrypt(nil)
	require.NoError(t, err)
	require.Equal(t, cipherText_nil, cipher)

	sk, err = aes_cbc_256.NewCrypto(sk_ei, iv, padding)
	require.NoError(t, err)
	cipher, err = sk.Encrypt(plainText)
	require.NoError(t, err)
...
func TestDecrypt(t *testing.T) {
	...
	sk, err = aes_cbc_256.NewCrypto(sk_ei, iv_nil, padding_nil)
	require.NoError(t, err)
	plain, err = sk.Decrypt(cipherText_nil)
	require.NoError(t, err)
	testnil := make([]byte, 0)
